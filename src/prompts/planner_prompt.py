...
# Change ACTIVE_VERSION to switch between prompt versions.
# All versions live in: src/prompts/versions/
# ---------------------------------------------------------------------------

ACTIVE_VERSION = "v2"

_VERSIONS_DIR = os.path.join(os.path.dirname(__file__), "versions")
_prompt_file = os.path.join(_VERSIONS_DIR, f"{ACTIVE_VERSION}_system_prompt.txt")

with open(_prompt_file, "r", encoding="utf-8") as _f:
    SYSTEM_PROMPT = _f.read()

# ---------------------------------------------------------------------------
# Legacy inline prompt kept below for reference only — NOT used when loading
# from a version file. Delete this block once v2 is validated.
# ---------------------------------------------------------------------------
...
- aggregate_column and groupby_aggregate must NEVER appear together in the same plan.
  ...
  * If the result should have one row per group (per year, per category, per region, which X, etc.)
    → use groupby_aggregate only. Never use aggregate_column.
  * If the result is a single overall number with no grouping dimension
    → use aggregate_column only. Never use groupby_aggregate.
